...
import { Shield, Lock, ChevronDown, ChevronUp, Save } from 'lucide-react';
...
export function PrivacySettings() {
  ...
  const [tempPrivacyLevel, setTempPrivacyLevel] = useState<PrivacyLevel>('private');
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      try {
        ...
        if (data) {
          const level = data.privacy_level || 'private';
          setPrivacyLevel(level);
          setTempPrivacyLevel(level);
        }
      } catch (error) {
  ...
  const handlePrivacyLevelChange = (newLevel: PrivacyLevel) => {
    setTempPrivacyLevel(newLevel);
    setHasChanges(newLevel !== privacyLevel);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const { error } = await supabase
        .from('subscribers')
        .update({ privacy_level: tempPrivacyLevel })
        .eq('user_id', user.id);

      if (error) throw error;

      setPrivacyLevel(tempPrivacyLevel);
      setHasChanges(false);
      ...
      toast({
        ...
        description: descriptions[tempPrivacyLevel],
      });
    } catch (error) {
          ...
          <>
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-lg border space-y-4">
                ...
                <RadioGroup value={tempPrivacyLevel} onValueChange={(value) => handlePrivacyLevelChange(value as PrivacyLevel)}>
